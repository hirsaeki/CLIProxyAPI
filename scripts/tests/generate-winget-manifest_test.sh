...
if [[ "$relative_file_count" -ne 2 ]]; then
  printf 'expected only two server executable entries, found %s\n' "$relative_file_count" >&2
  exit 1
fi
if grep -Fq -- 'sync-oauth-model-availability' "$installer_manifest"; then
  echo 'installer manifest unexpectedly exposes the optional OAuth model helper' >&2
  exit 1
fi
